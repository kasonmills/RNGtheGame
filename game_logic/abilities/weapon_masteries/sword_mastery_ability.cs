...
{
    /// <summary>
    /// Sword Mastery - Passive ability that provides bonuses when wielding a sword
    ...
    /// Only active when wielding a Sword weapon
    /// </summary>
    public class SwordMasteryAbility : Ability
    {
        public SwordMasteryAbility()
        {
            Name = "Sword Mastery";
        ...
        public bool IsWieldingCorrectWeapon(Entity entity)
        {
            if (entity is Entities.Player.Player player)
            {
                return player.EquippedWeapon != null &&
                       player.EquippedWeapon.Type == Items.WeaponType.Sword;
            }
            return false;
        }
        ...
        protected override void OnLevelUp()
        {
            if (Level % 10 == 0)
            {
                Console.WriteLine($"Your sword mastery improves! Now: +{GetAccuracyBonus()}% accuracy, +{GetAttackDamageBonus()}% damage, +{GetCritChanceBonus()}% crit chance!");
            }
        }
        ...
        public override string GetInfo()
        {
            string info = $"{Name} (Lv.{Level}/{MaxLevel}) [PASSIVE]\\n" +
                         $"{Description}\\n\\n" +
                         $"Current Bonuses (when wielding a Sword):\\n" +
            ...
            if (Level < 100)
            {
                ...
                Ability tempAbility = new SwordMasteryAbility();
                tempAbility.Level = nextMilestone;
                var temp = (SwordMasteryAbility)tempAbility;
        ...
        {
            Console.WriteLine($"{Name} is a passive ability providing bonuses when wielding a sword.");
