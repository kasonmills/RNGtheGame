...
{
    /// <summary>
    /// Mace Mastery - Passive ability that provides bonuses when wielding a mace
    /// Level 1: +1% accuracy, +1% attack damage, +1% crit chance
    /// Level 100: +40% accuracy, +70% attack damage, +25% crit chance
    /// </summary>
    public class MaceMasteryAbility : Ability
    {
        public MaceMasteryAbility()
        {
            Name = "Mace Mastery";
        ...
        public bool IsWieldingCorrectWeapon(Entity entity)
        {
            if (entity is Entities.Player.Player player)
            {
                return player.EquippedWeapon != null &&
                       player.EquippedWeapon.Type == Items.WeaponType.Mace;
            }
            return false;
        }
        ...
        protected override void OnLevelUp()
        {
            if (Level % 10 == 0)
            {
                Console.WriteLine($"Your mace mastery improves! Now: +{GetAccuracyBonus()}% accuracy, +{GetAttackDamageBonus()}% damage, +{GetCritChanceBonus()}% crit chance!");
            }
        }
        ...
        public override string GetInfo()
        {
            string info = $"{Name} (Lv.{Level}/{MaxLevel}) [PASSIVE]\\n" +
                         $"{Description}\\n\\n" +
                         $"Current Bonuses (when wielding a Mace):\\n" +
            ...
            if (Level < 100)
            {
                ...
                Ability tempAbility = new MaceMasteryAbility();
                tempAbility.Level = nextMilestone;
                var temp = (MaceMasteryAbility)tempAbility;
        ...
        {
            Console.WriteLine($"{Name} is a passive ability providing bonuses when wielding a mace.");
